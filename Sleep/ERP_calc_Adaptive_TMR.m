...
path_WM = [path_WM 'Data\Behavior\Adaptive_TMR'];

path_save = [];
for i=1:length(list)-2
    path_save = [path_save,list{i},'\'];
end
path_save = [path_save 'Analysis\Sleep\ERP\Adaptive_TMR'];
...
for n = 1:length(TMRNames)
    ...
    % Ori
    Adaptive_TMR_ERP = CuedBLData;
    Adaptive_TMR_TFR = CuedData;
    Adaptive_TMR_PAC = CuedData;

    Level = eventLevels;

    numLevel2 = sum(strcmp(eventLevels, '2'));
    numLevel3 = sum(strcmp(eventLevels, '3'));

    fprintf('Number of Level 2 for subject %d: %d\n', n, numLevel2);
    fprintf('Number of Level 3 for subject %d: %d\n', n, numLevel3);
    fprintf('\n');
    save(fullfile(path_save, matFiles(n).name), 'Adaptive_TMR_ERP','Adaptive_TMR_TFR','Adaptive_TMR_PAC', 'Level');
end
