...
for g = 1:length(groups)
    ...
    for s = 1:length(subjects)
        ...
        if isfield(loadedData, 'Adaptive_TMR_ALL_SO')
            if isfield(loadedData, 'Adaptive_TMR_ALL_trial_SO')
                data.Adaptive_TMR.All{end+1} = loadedData.Adaptive_TMR_ALL_trial_SO;
            end
            if isfield(loadedData, 'Adaptive_TMR_L3_trial_SO')
                data.Adaptive_TMR.L3{end+1} = loadedData.Adaptive_TMR_L3_trial_SO;
            end
        end
        if isfield(loadedData, 'TMR_ALL_SO')
            if isfield(loadedData, 'TMR_ALL_trial_SO')
                data.TMR.All{end+1} = loadedData.TMR_ALL_trial_SO;
            end
            if isfield(loadedData, 'TMR_L3_trial_SO')
                data.TMR.L3{end+1} = loadedData.TMR_L3_trial_SO;
            end
        end
        if isfield(loadedData, 'CNT_SO')
            if isfield(loadedData, 'CNT_trial_SO')
                data.CNT{end+1} = loadedData.CNT_trial_SO;  
            end
        end
    end
end
...
for idx = 1:length(levels)
    ...
    for s = 1:numSurrogate
        for t = 1:timePoints
            ...
            surrogateAccMatrix(:, :, s, t) = accMatrix; % Accuracy * 100
...
%% Save Results
savePath = fullfile(path_save, 'SW_SVM_rbf.mat'); 
...
function [data, labels] = prepareData(groupData, t)
    ...
    for g = 1:length(groupData)
        for s = 1:length(groupData{g})
            trialData = squeeze(mean(groupData{g}{s}(:, t, :, :), 3)); 
            % avg freq
